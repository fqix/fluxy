...
func elevateSetup(r setupRequest) error {
	...
	pipe := `\\.\pipe\` + protocol.ServiceID + ".setup." + hex.EncodeToString(nonce)
...
func performNativeSetup(r setupRequest) (resultErr error) {
	...
	s, err := manager.OpenService(protocol.ServiceID)
	...
	if s != nil {
		...
		if errors.Is(err, windows.ERROR_FILE_NOT_FOUND) {
			...
			for {
				probe, probeErr := manager.OpenService(protocol.ServiceID)
	...
	if s == nil {
		s, err = manager.CreateService(protocol.ServiceID, binary, mgr.Config{DisplayName: "Fluxy Helper", StartType: mgr.StartAutomatic, ServiceStartName: "LocalSystem"})
		if err != nil {
			return err
		}
		created = true
