...
func nativeInstallerResult(output []byte, marker string) error {
	...
	for _, line := range lines {
		if !strings.HasPrefix(line, marker) {
			diagnostics = append(diagnostics, strings.ReplaceAll(line, marker, "[completion]"))
		}
	}
	detail := strings.TrimSpace(strings.Join(diagnostics, "\n"))
	if !strings.HasPrefix(completion, marker) {
		return errors.New("native installer ended without a completion result: " + detail)
	}
	code, err := strconv.Atoi(strings.TrimPrefix(completion, marker))
	if err != nil || code < 0 || code > 255 {
		return errors.New("native installer returned an invalid completion result: " + detail)
	}
	if code != 0 {
		if detail == "" {
			return fmt.Errorf("native helper operation failed (exit status %d)", code)
		}
		return errors.New("native helper operation failed: " + detail)
	}
	return nil
}
