...
export function prepareRelease(directory, version, platform, arch) {
    ...
    // Match electron-builder/electron-updater's platform suffixes for latest-${arch}.
    const suffix = {
        darwin: '-mac',
        linux: arch === 'arm64' ? '-linux-arm64' : '-linux',
        win32: ''
    }[platform]
    const manifest = `latest-${arch}${suffix}.yml`
    ...
    const plan = { version, platform, arch, versioned, manifest }
    writeFileSync(join(directory, 'release-plan.json'), JSON.stringify(plan, null, 2) + '\n')
    return plan
}

// All platform builds share one versioned release.
export function publishRelease(directories, tag) {
    ...
    for (const { directory, plan } of entries) {
        ...
        for (const name of plan.versioned) {
