...
func startSplitDNS(name string, domains []string) (func() error, error) {
	...
	payload, _ := json.Marshal(dnsLeaseRequest{InterfaceName: name, DNS: splitDNSParams{Domains: domains}})
	return startWindowsDNSOwner(command, payload, func() error { return systemNRPT().remove(serviceID + "." + name) })
}

// Only the protected, elevated helper can enter the mutating lease command.
func platformDNSLease() error {
	if _, err := secureBase(); err != nil {
		return err
	}
	...
	if err := decode(line, &request); err != nil {
		return err
	}
	if !validInterfaceName(request.InterfaceName) {
