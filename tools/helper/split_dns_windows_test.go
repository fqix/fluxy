...
func TestNativeNRPTScopeAndCleanup(t *testing.T) {
	...
	for _, rule := range rules {
		...
		key.Close()
		if version != 2 || flags != 8 || server != splitDNSAddress || len(rule.domains) != 2 || rule.domains[1] != "."+rule.domains[0] {
...
func TestNativeDNSLeaseEOFAndRollback(t *testing.T) {
	for _, fail := range []bool{false, true} {
		t.Run(map[bool]string{false: "EOF", true: "activation failure"}[fail], func(t *testing.T) {
			...
			go func() {
				done <- runNativeDNSLease(reader, &output, s, dnsLeaseRequest{InterfaceName: "fluxy2345", DNS: splitDNSParams{Domains: []string{"example.com"}}})
			}()
...
func TestNativeDNSCleanupRetry(t *testing.T) {
	...
	if err := s.remove(serviceID + ".fluxy2345"); err == nil {
