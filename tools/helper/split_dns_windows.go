	...
	for {
		...
		for _, name := range names {
			...
			for _, address := range addresses {
				if !netip.MustParsePrefix("198.19.0.0/16").Contains(address) && !netip.MustParsePrefix("100.127.0.0/16").Contains(address) && !netip.MustParsePrefix("172.30.0.0/16").Contains(address) {
...
func ownedDNSInterface(owner string) string {
	name := strings.TrimPrefix(owner, serviceID+".")
...
func (s nrptStore) install(name string, domains []string) (func() error, error) {
	...
	for _, domain := range domains {
		...
		if err == nil {
			err = key.SetStringValue("GenericDNSServers", splitDNSAddress)
		}
		if err == nil {
...
func flushSplitDNSCache() { _ = flushWindowsDNS() }
func recoverWindowsSplitDNS() error {
	s := systemNRPT()
	rules, err := s.rules(s.local)
	if err != nil {
		return err
