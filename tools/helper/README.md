...
On macOS, a validated `splitDNS` TUN profile captures only a dedicated Fake IP pool and the internal DNS address. The helper waits for the internal DNS to answer before registering a supplemental resolver for the validated capture domains (the root suffix only when the list is empty). The same domain suffixes constrain Fake IP answers. A persistent `scutil` session owns a temporary Dynamic Store key; closing its stdin or losing the helper removes that key without overwriting another network service. No arbitrary resolver keys or shell commands are accepted over RPC. Split DNS process-lifecycle tests use a fixture process and do not alter host DNS.

The desktop Helper & Certificate Setup operation generates or reuses the local CA. On macOS, initial setup runs in one desktop native adapter: Authorization Services authorizes the existing launchd installer, the elevated installer inserts the public certificate, and the desktop process calls Security.framework for certificate trust while retaining its authorization reference. AppleScript is no longer used. The current manual service layout and unsigned Electron development host use the deprecated AuthorizationExecuteWithPrivileges compatibility API; migrating to SMAppService requires changing service packaging. This does not guarantee a single prompt: credential reuse and any additional trust approval remain controlled by macOS and require desktop acceptance testing. No private key leaves Fluxy. Setup verifies trust before reporting completion; canceling trust preserves the installed helper, and retry performs only unfinished steps. Other supported platforms use the helper's CA operation. Starting TUN never installs or updates a helper implicitly: missing or outdated helpers require explicit setup. Existing trusted certificates are reused without another trust mutation.
