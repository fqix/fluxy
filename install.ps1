<#
.SYNOPSIS
Downloads, verifies and installs a published Fluxy release on Windows.
.EXAMPLE
.\install.ps1 -Version 0.3.1
.EXAMPLE
.\install.ps1 -DryRun
#>
[CmdletBinding()]
param(
    [string]$Version = 'latest',
    [ValidateSet('auto', 'x64', 'arm64')][string]$Arch = 'auto',
    [switch]$DryRun,
    [string]$LogPath
)
# Keep installer preferences and working variables out of an interactive iex caller.
& {
    $ErrorActionPreference = 'Stop'
    $ProgressPreference = 'SilentlyContinue'
    if (!$LogPath) { $LogPath = Join-Path ([IO.Path]::GetTempPath()) 'fluxy-install.log' }
    $LogPath = [IO.Path]::GetFullPath($LogPath)
    [IO.File]::WriteAllText($LogPath, '')
    function Write-InstallLog([string]$Message) {
        [IO.File]::AppendAllText($LogPath, "$(Get-Date -Format o) $Message`r`n")
        Write-Output "Fluxy: $Message"
    }
    $work = $null
    $stage = 'initialization'
    try {
        Write-InstallLog "Installation log: $LogPath"
        Write-InstallLog "PowerShell $($PSVersionTable.PSVersion); process architecture: $env:PROCESSOR_ARCHITECTURE; native architecture: $env:PROCESSOR_ARCHITEW6432"
        if ($env:OS -ne 'Windows_NT') { throw 'Use install.sh on macOS and Linux.' }
        if ($Arch -eq 'auto') {
            $nativeArch = $env:PROCESSOR_ARCHITEW6432
            if (!$nativeArch) { $nativeArch = $env:PROCESSOR_ARCHITECTURE }
            switch ($nativeArch) {
                'AMD64' { $Arch = 'x64' }
                'ARM64' { $Arch = 'arm64' }
                default { throw 'Only x64 and arm64 are supported.' }
            }
        }
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
        $Version = $Version -replace '^v', ''
        if ($Version -eq 'latest') {
            $stage = 'resolving latest release'
            Write-InstallLog $stage
            $response = Invoke-WebRequest -UseBasicParsing -Method Head -Uri 'https://github.com/fqix/fluxy/releases/latest' -TimeoutSec 60
            # Windows PowerShell uses HttpWebResponse; PowerShell 7 uses HttpResponseMessage.
            $latestUri = $response.BaseResponse.ResponseUri
            if (!$latestUri) { $latestUri = $response.BaseResponse.RequestMessage.RequestUri }
            if (!$latestUri -or $latestUri.AbsoluteUri -notmatch '^https://github\.com/fqix/fluxy/releases/tag/v(.+)$') {
                throw 'Could not resolve the latest release.'
            }
            $Version = $Matches[1]
        }
        if ($Version -notmatch '^\d+\.\d+\.\d+(-[A-Za-z0-9.-]+)?$') { throw 'Invalid release version.' }
        $tag = "v$Version"
        $artifact = "Fluxy-$Version-win-$Arch.exe"
        $url = "https://github.com/fqix/fluxy/releases/download/$tag/$artifact"
        Write-InstallLog "Windows / $Arch / $tag`n$url`n$url.sha256"
        if ($DryRun) { return }
        $work = Join-Path ([IO.Path]::GetTempPath()) ('fluxy-install-' + [Guid]::NewGuid().ToString('N'))
        New-Item -ItemType Directory -Path $work | Out-Null
        $checksum = Join-Path $work 'checksum'
        $installer = Join-Path $work $artifact
        $stage = 'downloading checksum'
        Write-InstallLog $stage
        Invoke-WebRequest -UseBasicParsing -Uri "$url.sha256" -OutFile $checksum -TimeoutSec 60
        $expected = ((Get-Content -LiteralPath $checksum -TotalCount 1) -split '\s+')[0]
        if ($expected -notmatch '^[a-fA-F0-9]{64}$') { throw 'Invalid SHA-256 checksum file.' }
        $stage = 'downloading installer'
        Write-InstallLog $stage
        Invoke-WebRequest -UseBasicParsing -Uri $url -OutFile $installer -TimeoutSec 900
        $stage = 'verifying installer'
        Write-InstallLog $stage
        if ((Get-FileHash -Algorithm SHA256 -LiteralPath $installer).Hash -ne $expected) {
            throw 'Checksum mismatch; nothing was installed.'
        }
        $stage = 'running installer'
        Write-InstallLog $stage
        $installProcess = Start-Process -FilePath $installer -ArgumentList '/S' -Wait -PassThru
        Write-InstallLog "Installer exit code: $($installProcess.ExitCode)"
        if ($null -eq $installProcess.ExitCode) { throw 'Windows did not report an installer exit code.' }
        if ($installProcess.ExitCode -eq 3010) { Write-InstallLog 'Installed. Windows reports that a restart is required.' }
        elseif ($installProcess.ExitCode -ne 0) { throw "Fluxy installer failed (exit code $($installProcess.ExitCode))." }
        else { Write-InstallLog 'Installed. Open Fluxy from the Start menu.' }
    } catch {
        Write-InstallLog "Failed while ${stage}: $($_.Exception.Message)"
        throw "Fluxy installation failed while ${stage}. Log: $LogPath. $($_.Exception.Message)"
    } finally {
        if ($work -and (Test-Path -LiteralPath $work)) {
            Remove-Item -LiteralPath $work -Recurse -Force -ErrorAction Continue
        }
    }
}
